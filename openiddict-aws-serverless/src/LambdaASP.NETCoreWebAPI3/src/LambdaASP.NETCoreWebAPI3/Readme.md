...
WORKDIR /src
COPY ["LambdaASP.NETCoreWebAPI3.csproj", "LambdaASP.NETCoreWebAPI3/"]
RUN dotnet restore "LambdaASP.NETCoreWebAPI3/LambdaASP.NETCoreWebAPI3.csproj"

WORKDIR "/src/LambdaASP.NETCoreWebAPI3"
COPY . .
RUN dotnet build "LambdaASP.NETCoreWebAPI3.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "LambdaASP.NETCoreWebAPI3.csproj" \
...
Execute unit tests
```
    cd "LambdaASP.NETCoreWebAPI3/test/LambdaASP.NETCoreWebAPI3.Tests"
    dotnet test
```

Deploy application
```
    cd "LambdaASP.NETCoreWebAPI3/src/LambdaASP.NETCoreWebAPI3"
